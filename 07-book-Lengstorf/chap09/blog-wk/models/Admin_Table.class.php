<?php
  ...
  class Admin_Table extends Table {
    public function create(string $email, string $userid, string $password) {
      ...
      if (($this->validate_password($password)) and ($valid_email) and 
        (strlen($userid) >= 8)) {
        if ($this->checkAvailable($email, $userid)) {
          ...
          throw new Exception("E-mail and/or Userid already used.");
        }
      } else {
        throw new Exception("Userid and/or Password not valid.");
      }
    }
    ...
    private function validate_password(string $password) : bool {
      ...
      if ((strlen($length >=8)) and ($uppercase) and ($lowercase) and 
        ($number) and ($specialCharacters)) {
        return true;
      } else {
        return false;
      }
