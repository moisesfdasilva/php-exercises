<?php
  ...
  if ($xlsx = SimpleXLSX::parse('testdata.xlsx')) {
    ...
    foreach($rows as $row) {
      ...
      $lastname = $row[0];
      $firstname = $row[1]; $gender = $row[2];
      $assignmentaverage = $row[3];
      $discussionaverage = $row[4];
      $researchaverage = $row[5];
      $semesteraverage = $row[6];
      $semestergrade = $row[7];
      $entrySQL = "INSERT INTO student_data(lastname, firstname,
        gender, assignmentaverage, discussionaverage,
        researchaverage, semesteraverage, semestergrade) VALUES
        ( ?, ?, ?, ?, ?, ?, ?, ?)";
      $formData = array($lastname, $firstname, $gender,
        $assignmentaverage, $discussionaverage, $researchaverage,
