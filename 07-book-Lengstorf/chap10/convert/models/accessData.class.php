  ...
  class accessData {
    function returnDatabases() {
      ...
      foreach($databases as $database) {
        if(($database=="information_schema" or
          $database=="mysql" or $database=="performance_schema" or
          $database=="phpmyadmin")) {
          continue;
        }
    ...
    function returnDatabaseTables($database) {
      ...
      $data = "<input type='hidden' id='database' name='database'
        value='$database'>";
      $data .= "<select name='columns' id='columns'>";
    ...
    function returnDatabaseTitles($database, $table, $title, $flag=false) {
      ...
      $data = "<input type='hidden' id='database' name='database'
        value='$database'>";
      $data .= "<input type='hidden' id='table' name='table'
        value='$table'>";
      $data .= "<input type='hidden' id='title' name='title'
        value='$title'>";
    ...
    function returnExcelTitles($file, $title, $title_Name, $flag=false) {
      ...
      if ($xlsx = SimpleXLSX::parse($file)) {
        $rows = $xlsx->rows();
        $data = "<input type='hidden' id='numeric_Title'
          name='numeric_Title' value='$title'>";
        $data .= "<input type='hidden' id='numeric_Name'
          name='numeric_Name' value='$title_Name'>";
        $data .= "<input type='hidden' id='filename'
    ...
    function returnCSVTitles($file, $title, $title_Name, $flag=false) {
      ...
      if($file_to_read !== FALSE) {
        $data ="<input type='hidden' id='CSV_Numeric_Title'
          name='CSV_Numeric_Title' value='$title'>";
        $data .= "<input type='hidden' id='CSV_Numeric_Name'
          name='CSV_Numeric_Name' value='$title_Name'>";
        $data .= "<input type='hidden' id='CSV_Filename'
          name='CSV_Filename' value='$file'>";
        $data .= "<select name='columns' id='columns'>";
    ...
    function returnCSVData($file, $row, $col) {
      ...
      if($file_to_read !== FALSE) {
        ...
        while(!feof($file_to_read) && 
          ($line =fgetcsv($file_to_read)) !== false) {
          $lines[] = $line;
        }
        $i = 0;
    ...
    function returnJSONTitles($file, $title, $title_Name, $flag=false) {
      ...
      $data = "<input type='hidden' id='JSON_Numeric_Title'
        name='JSON_Numeric_Title' value='$title'>";
      $data .= "<input type='hidden' id='JSON_Numeric_Name'
        name='JSON_Numeric_Name' value='$title_Name'>";
      $data .= "<input type='hidden' id='JSON_Filename'
        name='JSON_Filename' value='$file'>";
      $data .= "<select name='columns' id='columns'>";
      // Display data
      $i = 0;
