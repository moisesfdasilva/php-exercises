      ...
      ($checker_board[$second_row][$second_column] == "black")) {
        if($second_row - $first_row == 2) {
          if($second_column - $first_column == 2) {
            ...
            if((($checker_board[$first_row + 1][$first_column + 1] != "black") && // not jumping empty space
              (substr($checker_board[$first_row +1][$first_column + 1],0,3) !=
              substr($checker_board[$first_row][$first_column],0,3)))) {
        ...
        if (($checker_board[$first_row] [$first_column] != "white checker") && 
          ...
          if($second_column - $first_column == -2) {
            // left side jump attempted
            if((($checker_board[$first_row-1][$first_column-1] != "black") && 
              (substr($checker_board[$first_row-1] [$first_column-1],0,3) !=
              substr($checker_board[$first_row] [$first_column],0,3)))) {
                // not jumping its own color
                if((second_row == 0) && ($checker_board[$first_row] [$first_column] == "white checker")) {
                  $checker_board[$second_row][$second_column] = "white king";
                } else {
                  $checker_board[$second_row][$second_column] = $checker_board[$first_row][$first_column];
                }
